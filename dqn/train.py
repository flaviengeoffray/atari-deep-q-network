...
def train(env: gym.Env, config: TrainingConfig = TrainingConfig()):
    ...
    agent = DQNAgent(
        q_network,
        env.action_space.n,
        epsilon_start=config.agent_epsilon_start,
        epsilon_decay=config.agent_epsilon_decay,
        epsilon_end=config.agent_epsilon_end,
        device=device,
    )
    ...
    for episode in range(1, nb_episodes + 1):
        ...
        for t in range(nb_steps_per_episode):
            ...
            if len(memory) >= batch_size and t % train_frequency == 0:
                ...
                next_state_batch = torch.FloatTensor(np.array(batch.next_state)).to(
                    device
                )
                ...
                if t % 100 == 0:
                    writer.add_scalar(
                        "Q-Values/Mean", q_values.mean().item(), step_count
                    )
        ...
        if loss_count > 0:
            ...
            print(
                f"Episode {episode} - Total Reward: {total_reward:.2f}, Avg Loss: {avg_loss:.4f}, Epsilon: {agent.epsilon:.4f}"
            )
        else:
            print(
                f"Episode {episode} - Total Reward: {total_reward:.2f}, No training step performed, Epsilon: {agent.epsilon:.4f}"
            )
...
def evaluate(
    env: gym.Env, agent: DQNAgent, n_episodes: int = 5, create_video=True
) -> float:
    ...
    if create_video:
        ...
        print(f"Recording evaluation video(s) to {video_folder}")
